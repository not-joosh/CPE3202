...
bool isNegative(unsigned char operand);
...
void main(void)
{
    ...
    ALU(0xC0, 0x0A, multiplication);  // 11000000 * 00000010 (multiply) = 192 => 11000000
    // ALU(0x05,0x05, multiplication); // 5 * 5 (multiply 4 bits to 8 bit result) = 35 => 0000000000100011    
...
void boothsAlogrithm(unsigned char *A, unsigned char M, unsigned char Q) { 
    ...
    for(n = 0; n < 9; n++){
        ...
        *A &= 0x7F; // Clear the MSB of A
        ...
        if (MSB_A != 0) *A |= 0x80; // Set MSB to 1 if MSB_A is 1
...
/*===============================================
*   FUNCTION    :   isNegative
*   DESCRIPTION :   Checks if the most significant bit (MSB) is negative
